// Pre-fill selected package from URL params (e.g. ?package=8-Session+Package&price=240)
(function() {
  var params  = new URLSearchParams(window.location.search);
  var pkg     = params.get('package');
  var price   = params.get('price');
  if (pkg) {
    var label   = pkg + (price ? ' — $' + price : '');
    var field   = document.getElementById('selected-package');
    var banner  = document.getElementById('package-banner');
    var bannerT = document.getElementById('package-banner-text');
...
const DB_API_URL = '/api/save-registration.php'; // Ionos MySQL endpoint
...
document.getElementById('registration-form').addEventListener('submit', function(e) {
  ...
  emailjs.send(EMAILJS_SERVICE, EMAILJS_TEMPLATE_WELCOME, emailData)
    .then(function() {
      ...
      status.textContent = 'Registration sent successfully! We will be in touch soon.';
