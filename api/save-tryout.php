...
$stmt = $link->prepare(
    'INSERT INTO tryout_registrations
        (first_name, last_name, age, dob, parent_name, parent_phone, parent_email,
         player_level, preferred_days, preferred_time, referral, assigned_coach, medical_notes, goals, waiver_accepted)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
);
...
$referral       = htmlspecialchars(trim($data['referral'] ?? ''), ENT_QUOTES, 'UTF-8');
...
$stmt->bind_param(
    ...
    $referral, $assigned_coach, $medical_notes, $goals, $waiver
);
